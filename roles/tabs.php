<?php // $Id: tabs.php,v 1.2 2010/08/23 10:41:33 Shtifanov Exp $
...
// require_once($CFG->wwwroot.'/blocks/monitoring/lib_auth.php');
// context definitions
...
if ($currenttab != 'update') {
    switch ($context->contextlevel) {
        case CONTEXT_REGION:
            ...
            $navlinks[] = array('name' => $strreportregion, 'link' => $CFG->wwwroot.'/blocks/monitoring/region/region.php', 'type' => 'misc');
        ...
        case CONTEXT_SCHOOL:
        	$strschoolss = get_string('schools', 'block_monitoring');
        	$strtitle = get_string('title','block_monitoring');
        	
            $navlinks[] = array('name' => $strtitle, 'link' => $CFG->wwwroot.'/blocks/monitoring/index.php', 'type' => 'misc');
            $navlinks[] = array('name' => $strschoolss, 'link' => null, 'type' => 'misc');
            $navigation = build_navigation($navlinks);
            print_header($SITE->fullname, $SITE->fullname, $navigation);
            break;
		...
		case CONTEXT_DISCIPLINE:         
			$strtitle0 = get_string('title','block_mou_school');  
        	$strtitle  = get_string('classlist','block_mou_school');
            $navlinks[] = array('name' => $strtitle0, 'link' => $CFG->wwwroot."/blocks/mou_school/index.php", 'type' => 'misc');
			$navlinks[] = array('name' => $strtitle, 'link' =>  $CFG->wwwroot."/blocks/mou_school/class/classlist.php?rid=0&amp;sid=0&amp;yid=0", 'type' => 'misc');   
            $navlinks[] = array('name' =>  get_string('roles'), 'link' => null, 'type' => 'misc');
            $navigation = build_navigation($navlinks);
            print_header($SITE->shortname, $SITE->fullname, $navigation);
			break;
