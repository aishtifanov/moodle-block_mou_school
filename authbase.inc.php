<?php // $Id: authbase.inc.php,v 1.4 2011/04/14 13:02:57 shtifanov Exp $
	...
	if (!$strlistrayons && !$strlistschools)   { 
 	...
 	if ($action != 'excel' && $action != 'word') 	{
		$strtitle = get_string('title','block_mou_school');
		$strscript = get_string($arrscriptname[0],'block_mou_school');
	
	    $navlinks = array();
	    $navlinks[] = array('name' => $strtitle, 'link' => "$CFG->wwwroot/blocks/mou_school/index.php?rid=$rid&amp;yid=$yid&amp;sid=$sid", 'type' => 'misc');
