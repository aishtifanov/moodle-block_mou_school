<?PHP // $Id: delreview.php,v 1.3 2010/08/23 08:47:57 Shtifanov Exp $
    ...
    $strreview = get_string('deleteprofilereview','block_mou_school');
	...
	$breadcrumbs .= "-> $strreview";
    print_header("$SITE->shortname: $strreview", $SITE->fullname, $breadcrumbs);
...
*/
 	if ($delete) {              // Delete a selected user, after confirmation
        ...
        // }
		$redirlink = "{$CFG->wwwroot}/blocks/mou_school/class/review.php?mode=4&amp;rid=$rid&amp;sid=$sid&amp;yid=$yid&amp;gid=$gid&amp;uid=$uid";
        ...
        if ($confirm != md5($delete)) {
            ...
            print_heading(get_string('deleteprofilereview', 'block_mou_school'));
            $optionsyes = array('rid'=>$rid, 'sid'=>$sid, 'yid'=>$yid, 'gid'=>$gid, 'portid'=>$portid,'uid'=>$uid,
            					'confirm'=>md5($delete), 'mode'=>4,);
	        notice_yesno(get_string('deletecheckfullaction', 'block_mou_school', "'$fullname'"), 'delreview.php', $redirlink, $optionsyes, $optionsyes, 'post', 'get');

        } else {

           		delete_records('monit_school_portfolio', 'id', $portid);
		   		redirect($redirlink, get_string('reviewdeleted', 'block_mou_school'), 2);
        }
    }
